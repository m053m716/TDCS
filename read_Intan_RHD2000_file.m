function read_Intan_RHD2000_file(fname,suppressAmp,stimFile)
%READ_INTAN_RHD2000_FILE  Read binary file with tDCS data streams
%
%  >> read_Intan_RHD2000_file;
%  >> read_Intan_RHD2000_file(fname);
%  >> read_Intan_RHD2000_file(fname,suppressAmp);
%  --> If suppressAmp is true, (default: false), then no amplifier data
%      is returned and instead it just saves the time of stim onset in the
%      block folder.
%
...
if nargin < 3
   stimFile = '';
   saveStimTimeOnly = false;
elseif isempty(stimFile)
   saveStimTimeOnly = false;
else
   saveStimTimeOnly = true;
end
...
if nargin < 1
   [file, path, ~] = ...
      uigetfile('*.rhd',...
      'Select an RHD2000 Data File',...
      defs.FileNames('RAW_BINARY_LOCATION'), ...
      'MultiSelect', 'off');
   ...
   [path,f,e] = fileparts(fname);
   if isempty(path)
      path = defs.FileNames('RAW_BINARY_LOCATION'); % Default location
   end
   
   if isempty(e)
      e = '.rhd';
   elseif ~strcmpi(e,'.rhd')
      error(['tDCS:' mfilename ':BadFileType'],...
         ['\n\t->\t<strong>[TDCS.READ_INTAN_RHD2000_FILE]:</strong> ' ...
         'Wrong filetype (''%s''). This function requires ''.rhs''\n'],e);
   end
   file = [f e];
   if exist(fullfile(path,file),'file')~=2
      error(['tDCS:' mfilename ':BadFile'],...
         ['\n\t->\t<strong>[TDCS.READ_INTAN_RHD2000_FILE]:</strong> ' ...
         'File does not exist.\n\t\t\t(''%s'')\n'],fullfile(path,file));
   end
end
...
if (data_present)
   ...
   for i=1:num_data_blocks
      ...
      if (num_amplifier_channels > 0)
         ...
         else % Otherwise, maintain position in record
...
if (data_present)
   ...
   if (notch_filter_frequency > 0) && ~suppressAmp
